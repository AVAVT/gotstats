...
import { OGS_API_PLAYER_ROOT } from "./api-constants";

const OGSApi = {
  fetchUserId: (user: string) => {
    return new CancelablePromise<number>(async (resolve, reject) => {
      if (Number.isNaN(Number(user))) {
        // Entered username
        try {
          const { data } = await axios.get(`${OGS_API_PLAYER_ROOT}?username=${user}`);

          if (data.results.length > 0) {
            resolve(Number(data.results[0].id));
          } else {
            reject(
  ...
  fetchUserDataById: (id: number) => {
    return new CancelablePromise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(`${OGS_API_PLAYER_ROOT}${id}`);
        resolve(data);
      } catch (err) {
        if (err instanceof AxiosError) {
          reject(
